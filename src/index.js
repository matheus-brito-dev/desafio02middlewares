...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if(user.pro === false && user.todos.length === 10){
      return response.status(403).json({error: "Free plan is only 10 todos"});
  }
...
function checksTodoExists(request, response, next) {
  ...
  const checkId = validate(id); //método para validar uuid
  if(!checkId) return response.status(400).json({error: "Id is not a uuid"});
